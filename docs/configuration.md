...
| `BB_THREAD_TITLE_REFINE` | server environment                              | Optional                | Auto-name threads from the first *turn*. Defaults to `true`: local threads skip the first-message title and every thread (re)generates its title from the prompt plus the agent's first response once that turn completes; a manual rename is never overwritten. Set to `false` to name threads from the first message at creation instead. |
