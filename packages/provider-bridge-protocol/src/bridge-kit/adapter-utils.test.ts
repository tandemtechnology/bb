...
describe("adapter-utils", () => {
  ...
  it("builds set and unset shell environment policy directives", () => {
    expect(
      buildShellEnvironmentPolicyConfig({
        envVars: { BB_THREAD_ID: "thr_1", CLAUDE_CODE_USE_VERTEX: "1" },
        unsetVars: ["ANTHROPIC_API_KEY", "CLAUDE_CODE_USE_VERTEX"],
      }),
    ).toEqual({
